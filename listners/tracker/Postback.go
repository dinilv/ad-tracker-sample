...
func (postback *Postback) Tracker(ctx context.Context, req *api.Request, rsp *api.Response) error {
	...
	if transOK == false || len(transactionID) == 0 {
		//forward to postbackAPI without transaction with GET
		...
		header := map[string]*api.Pair{
			"Location": {
				Key:    "Location",
				Values: []string{"http://localhost:8000/aff_l/" + requestParams[constants.URL]},
			},
		}
