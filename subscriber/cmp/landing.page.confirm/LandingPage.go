...
func main() {
	...
	router.Initialize(200, 5, common.LandingPageSub, InsertBatch)

	//schedule for 5 secs loop
	go router.ProcessingBatch()

	//create a listner for shutdown
	apiRouter := router.InitRoutes("landing-page")
	server := &http.Server{
...
func InsertBatch(messages []*pubsub.Message) {
	...
	for _, msg := range messages {
		...
		logs := processor.SubscribeLandingPage(msg.Attributes)
	...
	if len(filteredTrackerLog) > 0 {
		dao.InsertManyToMongo(common.MongoDB, common.FilteredLandingPageLog, filteredTrackerLog)
	}
	//insert to landing page log
	if len(trackerLog) > 0 {
		dao.InsertManyToMongo(common.MongoDB, common.LandingPageLog, trackerLog)
	}
}
